...
#include "maps.h"
#include "headers/shadersTypes.h"
...
class CShaders : public CEngineBase
{
  ...
  public:
    ...
    inline const CShader *getVertexShader(uint32 index) const { return &vertexShaders.at(index); }
    inline const CShader *getFragmentShader(uint32 index) const { return &fragmentShaders.at(index); }
    inline const CShaderProgram *getShaderProgram(NShader::EProgram index) const { return &shaderPrograms.at(index); }
};
//------------------------------------------------------------------------------
